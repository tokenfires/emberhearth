...
struct PermissionsView: View {
    ...
    var body: some View {
        ...
        .onChange(of: permissionManager.currentStatus) { newValue in
            ...
            if (!previousStatus.automation && newValue.automation)
                || (!previousStatus.fullDiskAccess && newValue.fullDiskAccess) {
        ...
        return HStack(spacing: 16) {
            ...
            if isGranted {
                ...
                Text("Skipped")
                    ...
                    .fontWeight(.medium)
                    .foregroundStyle(.secondary)
            } else if permissionManager.notificationAuthorizationStatus == .denied {
                // User previously denied — can only fix in System Settings.
                VStack(spacing: 4) {
                    Button("Open Settings") {
                    ...
                    .accessibilityHint("Opens Notification settings where you can enable EmberHearth notifications")
                    .accessibilityIdentifier("onboarding_permissions_openNotificationSettingsButton")
                ...
                // Status is .notDetermined — can prompt.
                VStack(spacing: 4) {
                    Button("Enable") {
                        Task {
                            ...
                            await permissionManager.checkAllPermissions()
                            // If still not granted after the delay, the user denied.
                            // Mark handled so the buttons don't reappear.
                            if !permissionManager.currentStatus.notifications {
                                notificationHandled = true
                            }
                        }
                    }
